...
M.local_chatbot = {
  ...
  addWelcomeMessage: function () {
    ...
    this.addMessage(
      "bot",
      welcomeMessages[this.currentLanguage] || welcomeMessages["en"]
    );
  },
  ...
  // Updated method to return a promise for proper chaining
  callAPIWithJSONPromise: function (endpoint, data) {
  ...
  handleAPIError: function (error) {
    ...
    if (error.message.includes("CORS")) {
      errorMessage =
        "Connection blocked by CORS policy. Please check if the backend server is running on the correct URL: " +
        this.config.apiUrl;
    } else if (error.message.includes("Failed to fetch")) {
      errorMessage =
        "Cannot connect to the server. Please check if the backend is running at: " +
        this.config.apiUrl;
  ...
  addMessage: function (sender, text, isFile) {
    ...
    if (isFile) {
      content.innerHTML =
        '<div class="file-info">📎 ' + this.escapeHtml(text) + "</div>";
    } else {
  ...
  resetChat: function () {
    ...
    this.callAPIWithJSONPromise("/reset_session", requestData)
      .then(function (data) {
        // Clear chat messages
  ...
  // Updated changeLanguage method with auto-reload
  changeLanguage: function (newLang) {
    if (newLang === this.currentLanguage) return;

    // Store the new language immediately
    ...
    // Add loading message
    this.addMessage("bot", loadingMessages[newLang] || loadingMessages["en"]);

    // Set a small delay to show the loading message, then reload
    setTimeout(function () {
